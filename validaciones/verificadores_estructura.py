...
def validar_columnas_df(dfs_dict: dict[str, pd.DataFrame], columns_dict: dict[str, list]) -> Tuple[dict | None, bool]:
    """
    ...
    - Un diccionario con:
        ...
        - DataFrame de ventas
    - Un diccionario con las columnas
    ### Devuelve:
    ...
    for nombre_df, df in dfs_dict.items():
        ...
        if nombre_df in columns_dict:
            faltantes = set(columns_dict[nombre_df]) - set(df.columns) # Una genialidad de los sets!
            if faltantes:
